...
func methodChain2() {
	result := builder.SetVeryLongConfigurationOption(true).EnableAnotherFeatureFlag(false).ConfigureAdvancedSettings(opts).Build()
	_ = result
}

// Example 3: Method chain on receiver expression (from example.go_ line 468-471)
...
func methodChain4() {
	result := factory.CreateBuilder().WithOption1(val1).WithOption2(val2).WithOption3(val3).WithOption4(val4).Finalize().Build()
	_ = result
}

// Example 5: Method chain with struct literal argument
func methodChain5() {
	result := client.Configure(Config{Timeout: 30 * time.Second, MaxConnections: 3}).Execute(ctx, nil)
	_ = result
}

// Example 6: Method chain returning error
func methodChain6() error {
	return processor.Initialize(ctx).LoadConfig(configPath).ValidateInputs(inputs).ProcessAll().Finalize()
}
...
func structLiteral2() {
	server := NewServer(Config{Host: "localhost", Port: 8080, Timeout: 30 * time.Second, MaxConnections: 100, EnableTLS: true, CertPath: "/path/to/cert"})
	_ = server
}
...
func structLiteral4() {
	handler := Handler{OnSuccess: func(result string) { processResult(result) }, OnError: func(err error) { logError(err) }}
	_ = handler
}
...
// Example 17: Assignment with long selector expression (from example.go_ line 1860-1861)
...
func assignment2() {
	estimator := missionctrl.NewProbabilityEstimator(missionctrl.DefaultConfig())
	_ = estimator
}
...
func appendExample1(addrs []*NetAddress, update *NodeUpdate, net Network) []*NetAddress {
	return append(addrs, &NetAddress{IdentityKey: update.IdentityKey, Address: update.Address, ChainNet: net})
}

// Example 21: Append with multiple elements
func appendExample2(items []string) []string {
	return append(items, "item1", "item2", "item3", "item4", "item5", "item6", "item7", "item8")
}
...
func complexIf1(cache *Cache) {
	if cache.Get(key).IsValid() && cache.Get(key).NotExpired() && cache.Get(key).HasPermission(user) {
		process()
	}
}
...
func errorHandling1() error {
	return fmtpkg.Errorf("failed to initialize component %s with config %v: %w", componentName, config, err)
}

// Example 25: Multiple error checks in sequence (cleanup pattern from example.go_)
...
func funcLiteral1() {
	processFn(func(ctx context.Context, data []byte, opts Options) (Result, error) { return Result{}, nil })
}

// Example 28: Multiple function literal arguments
func funcLiteral2() {
	handle(func() error { return nil }, func() error { return nil }, func() error { return nil })
}
