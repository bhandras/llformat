...
func example11(f *formatter, trimmedLongVariable string, inSwitch,
	inInterface int) {
...
func example14(itemType string) {
	for {
		if itemType != "" && itemType != "text" &&
			itemType != "allowed_input_text" &&
			itemType != "allowed_output_text" {
...
func example15(firstConditionName, secondConditionName, thirdConditionName,
	fourthConditionName, fallbackEnabled bool) {

	if (firstConditionName && secondConditionName) ||
		(thirdConditionName && fourthConditionName) || fallbackEnabled {
...
func example17(items []int, stopRequested bool, ctx context.Context, retryCount,
	maxRetries int) {
