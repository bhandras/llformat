...
// Code below should remain unchanged: var x = 1 // trailing comment trailing
// comment
var x = 1
...
func demo() {
	...
	if true {
		...
		*/
		// This trailing comment is extremely long and should remain
		// exactly as-is without being wrapped by the formatter even
		// though it exceeds the configured width significantly in
		// practice.
		val := "keep"
		// inline block comment that must remain unchanged even if it's
		// very very very very long to test behavior
		keep := 1
}
	...
	if true {
		for i := 0; i < 1; i++ {
			...
			case 0:
				...
				//   continuation lines are emitted. this
				//   trailing inline comment at deep nesting
				//   level should be moved above when inline
				//   wrapping is enabled and it is long enough
				//   to need wrapping in that mode
				doSomething()
			}
		}
	}
