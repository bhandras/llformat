...
func main() {
	...
	result := someFunction(
		veryLongArgument1, veryLongArgument2, veryLongArgument3,
		veryLongArgument4,
	)
	_ = result
	...
	ProcessData(
		ctx, "some long string argument that makes the line exceed "+
			"limit", 42, true, time.Now(), &config,
	)

	// Nested function calls
	result2 := OuterFunction(
		InnerFunction(arg1, arg2, arg3), anotherLongArgument,
		yetAnotherArgument,
	)
	_ = result2
	...
	ProcessAsync(
		func() error { return nil }, onSuccess, onFailure, timeout,
		retries,
	)
	...
	json.Marshal(
		complexObject, options, formatting, validation, errorHandling,
	)
	...
	response := client.Post(
		"https://api.example.com/endpoint", "application/json",
		requestBody, headers, timeout,
	)
	_ = response

	// Error handling calls
	handleError(
		errors.New("something went wrong"), context.Background(),
	...
	for i := 0; i < 3; i++ {
		if i%2 == 0 {
			if i > 1 {
				ProcessData(
					ctx, "this is a very long string "+
						"that should be split and "+
						"followed by arguments neatly",
					i, true, time.Now(), &config,
				)
			}
		}
	}
	...
	res := Do2(
		Outer2(
			Inner2(
				"a deeply nested long string that will "+
					"likely wrap nicely",
			),
		),
		map[string]interface{}{
