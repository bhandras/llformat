...
func example12NonTargets(mylog interface{ Infof(string, ...interface{}) }) {
	...
	log.Info("non-f variant should be ignored, even if long long long " +
...
func example31NoSpaceUnicode() {
	log.Infof("測試測試測試測試測試測試測試測試😅😅😅😅😅😅😅😅😅😅測試測" +
		"試測試測試測試測試測試測試")
}
...
func example32Combining() {
	log.Infof("Café and déjà vu occur in this sentence repeatedly to " +
...
// accounting.
func example43Mix() {
	log.Infof("混合🙂é and A B with long tail to enforce split here and "+
		"still keep expressions:", 123)
}

// example44: Nested targeted call as first argument content.
func example44NestedFirst() {
	log.Infof(fmt.Sprintf("A very very long head that will wrap inside "+
		"nested call while still being attached correctly to the "+
		"outer call: %d %d", 1, 2))
}
