...
// ScanBalancedParen finds the matching ')' for '(' at position open. Skips over
// strings and comments. Returns the position of the closing ')', or -1 if not
// found.
func ScanBalancedParen(src []byte, open int) int {
	return ScanBalanced(src, open, '(', ')')
}

// ScanBalanced finds the matching close bracket for the open bracket at
// position. Generic version supporting any bracket pair. Returns -1 if not
// found.
