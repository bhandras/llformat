...
- Architecture highlights: CLI in `cmd/llformat/main.go`; comment formatting in `formatter/comment_formatter.go`; call formatting in `formatter/left_flow_call_formatter.go`; tests compare `input.go` → `output.go` goldens under `testdata/`.
