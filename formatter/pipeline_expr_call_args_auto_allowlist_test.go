...
func TestPipelineDSLExpr_AutoCallArgs_OnlyForExcludedCallees_UserList(t *testing.T) {
	t.Parallel()

	const in = `package p

func f(a, b, c, d, e, f2, g bool) {
	// foo is excluded from multiline call formatting, so AutoDSLCallArgs should
	// allow breaking inside its argument expressions.
	_ = foo(a && b && c && d && e && f2 && g)

	// bar is not excluded, so AutoDSLCallArgs should not break inside its args.
...
func TestPipelineDSLExpr_AutoCallArgs_AllowsNestedGenericCalleeBreaks(t *testing.T) {
	t.Parallel()
...
`
	...
	// Ensure we can break both the nested call's callee type args and the nested
	// logical chain, while keeping the `](` coupling intact.
