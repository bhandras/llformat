...
func TestIndentFromLine(t *testing.T) {
	...
	for _, tt := range tests {
		indent := IndentFromLine(
			[]byte(tt.src), tt.lineStart, tt.tabStop,
		)
		if indent.Base != tt.wantBase {
			t.Errorf("IndentFromLine(%q, %d, %d).Base = "+
				"%q, want %q", tt.src, tt.lineStart, tt.tabStop,
				indent.Base, tt.wantBase)
		}
		if indent.Width() != tt.wantWidth {
			t.Errorf("IndentFromLine(%q, %d, %d).Width() = "+
				"%d, want %d", tt.src, tt.lineStart, tt.tabStop,
				indent.Width(), tt.wantWidth)
		}
	}
}
...
func TestIndentWithExtra(t *testing.T) {
	...
	if extra.Width() != 10 {
		t.Errorf("WithExtra(\"  \").Width() = %d, want 10",
			extra.Width())
	}
}
...
func TestIndentFitsContent(t *testing.T) {
	...
	if !indent.FitsContent(long70, cfg) {
		t.Error(
			"FitsContent should return true for 70 chars with " +
				"8-width indent",
		)
	}
	...
	if indent.FitsContent(long73, cfg) {
		t.Error(
			"FitsContent should return false for 73 chars with " +
				"8-width indent",
		)
	}
}
