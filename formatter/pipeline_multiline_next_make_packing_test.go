...
func TestPipelineNext_MultiLineCalls_MakeAssignment_PacksShortArgsTogether(
	t *testing.T) {

	// Regression test for the `make([]T, 0, len(...))` pattern: when
	// multiline is required, keep the common `0, len(...)` packed on the
...
`
	...
	require.Contains(
		t, out, "		[][]byte, 0, len(xs),\n", "expected "+
			"packed args to stay on the same continuation line "+
			"when they fit",
	)
	require.NotContains(
		t, out, "		[][]byte,\n",
