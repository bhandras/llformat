...
func FormatCompositeLiteralArg(arg, contIndent string,
	...
	if isKV {
		// Keyed literals (map/struct): one entry per line with trailing
		// comma.
		for _, e := range elems {
		...
		// Slices/arrays: keep inline if they fit; otherwise pack
		// greedily. Note: Unlike keyed maps/structs, we don't
		// force-expand slices even when the containing call has
		// multiline elements - slices stay inline if they fit.
		...
		for i, e := range elems {
			...
			if firstInLine {
				...
				if lineLen+need <= columnLimit {
					...
					// line break, add trailing comma to
					// previous line
					b.WriteByte(',')
					b.WriteByte('\n')
					b.WriteString(innerIndent)
		...
		case '{':
			if paren == 0 && bracket == 0 && brace == 0 {
				...
				if isInterfaceBefore(s, i) {
					// Let the outer scan continue; this '{'
