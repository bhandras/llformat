...
func TestPipelineNext_MultiLineCalls_DefaultsToPackedForGenericCalls(
	t *testing.T) {
...
`
	...
	// Packed multiline: callee + "(" on the first line, then args packed,
	// then closing paren on its own line.
	require.Contains(
		t, out, "	_ = "+
...
`
	...
	require.Contains(
		t, out, "	_ = short(a, b, c)",
		"expected short calls to remain single-line",
	)
	require.NotContains(
		t, out, "short(\n",
...
`
	...
	p := NewPipeline(PipelineConfig{
		...
		UseDSLMultiLineCalls: true,
		DSLMultiLineStyle:    "",
		// Keep log call stage disabled so this test proves
		// multiline-calls does not rewrite printf-style calls on its
		// own.
		UseDSLLogCalls:   false,
		UseDSLExpr:       false,
		UseDSLComments:   false,
