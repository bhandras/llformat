...
func dslBudgetNext() dsl.RewriteBudget {
	...
	// Large enough to never trigger for normal formatting runs, but small
	// enough to act as a safety valve against pathological rules.
...
func ownedSpansForCalls(src []byte,
	match func(*ast.CallExpr) bool) llast.OffsetSpanSet {
	...
	ast.Inspect(file, func(n ast.Node) bool {
		...
		// Own the call expression and its argument list so earlier
		// stages do not rewrite within regions that this stage will
		// later format.
...
func buildCommentStageFormatter(stageName string, cfg BaseConfig,
	opts StageOptions, plan StagePlan, bundle DSLBundle) Formatter {
...
func buildCompactCallStageFormatter(stageName string, cfg BaseConfig,
	opts StageOptions, plan StagePlan, bundle DSLBundle) Formatter {
	...
	return NewDSLExprFormatter(DSLExprConfig{
		...
		OwnedSpansFunc: func(src []byte) llast.OffsetSpanSet {
			// Align ownership boundaries with the log/printf DSL
			// stage selection.
...
func buildExpressionStageFormatter(stageName string, cfg BaseConfig,
	opts StageOptions, plan StagePlan, bundle DSLBundle) Formatter {
...
func buildMultiLineCallStageFormatter(stageName string, cfg BaseConfig,
	opts StageOptions, plan StagePlan, bundle DSLBundle) Formatter {
...
func buildBlankLineStageFormatter(stageName string, cfg BaseConfig,
	opts StageOptions, plan StagePlan, bundle DSLBundle) Formatter {
