...
func ValidatePipelineConfig(cfg PipelineConfig) error {
	...
	validateOptionalEnum(&issues, "DSLExprLogicalStyle", cfg.DSLExprLogicalStyle, allowedDSLExprStyles())
	validateOptionalEnum(&issues, "DSLExprArithmeticStyle", cfg.DSLExprArithmeticStyle, allowedDSLExprStyles())
	validateOptionalEnum(&issues, "DSLExprCaseClauseStyle", cfg.DSLExprCaseClauseStyle, allowedDSLExprStyles())
	validateOptionalEnum(&issues, "DSLExprSelectorChainStyle", cfg.DSLExprSelectorChainStyle, allowedDSLExprStyles())

	if cfg.StagePlanOverride != nil {
		validateStageMode(&issues, "StagePlanOverride.Comments", cfg.StagePlanOverride.Comments)
		validateStageMode(&issues, "StagePlanOverride.LogCalls", cfg.StagePlanOverride.LogCalls)
		validateStageMode(&issues, "StagePlanOverride.Expressions", cfg.StagePlanOverride.Expressions)
		validateStageMode(&issues, "StagePlanOverride.MultiLineCalls", cfg.StagePlanOverride.MultiLineCalls)
		validateStageMode(&issues, "StagePlanOverride.Signatures", cfg.StagePlanOverride.Signatures)
		validateStageMode(&issues, "StagePlanOverride.BlankLines", cfg.StagePlanOverride.BlankLines)
	}

	// Today Allow+Auto is well-defined (Allow wins), so this is not an error.
	// Still, it's almost always a mistake, so surface a warning-like issue.
...
func validateOptionalEnum(issues *[]configIssue, field string, value string, allowed []string) {
	if value == "" {
		return
	}
