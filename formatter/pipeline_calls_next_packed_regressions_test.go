...
func TestPipelineNext_PackedMultiLine_GenericCallInReturnListUnderSwitch(
	t *testing.T) {
...
`
	...
	// The packed formatter should keep `return nil, <callee>(` together on
	// the first line and then pack args on continuation lines.
	require.Contains(
		t, out, "		return nil, "+
...
func TestPipelineNext_PackedMultiLine_GenericCallInAssignmentUnderIf(
	t *testing.T) {
...
`
	...
	require.Contains(
		t, out, "		x := "+
			"someVeryLongCalleeNameForTestingPurposes("+
			"\n			", "expected packed "+
			"multiline call formatting under an if statement",
	)
	require.Contains(
		t, out, "\n		)\n",
		"expected closing paren on its own line",
...
func TestPipelineNext_PackedMultiLine_IsIdempotentForGenericCalls(
	t *testing.T) {
...
`
	...
	require.Equal(
		t, out1, out2,
		"expected packed multiline call formatting to be idempotent",
	)
}
