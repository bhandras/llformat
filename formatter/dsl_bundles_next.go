...
func dslRulesForExpr(opts StageOptions) []dsl.Rule {
	...
	// - logical/arithmetic/case: legacy packed breaking unless explicitly set
	...
	// literals and single-line function literals.
...
func dslRulesForMultiLineCalls(opts StageOptions) (rules []dsl.Rule, nodeOrder dsl.NodeOrder) {
	...
	switch style {
	case "packed":
		rules = dsl.PackedMultiLineOnlyRulesWithOptions(optsCommon, packedFallback)
	case "packed-chain":
		rules = dsl.MultiLineCallRulesWithOptions(optsCommon, packedFallback)
	case "layout-args":
		o := optsCommon
...
func dslRulesForSignatures(opts StageOptions) []dsl.Rule {
	// Signature selection is intentionally conservative: native rules are opt-in.
...
func dslRulesForBlankLines(opts StageOptions) []dsl.Rule {
	...
	// multiline, insert a blank line before the first statement in the body/case.
	rules = append(rules,
		dsl.Rule{
