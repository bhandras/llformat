...
func TestCallRuleMatch(t *testing.T) {
	rule := &CallRule{
		Patterns: []string{"log.Infof(", "log.Errorf("},
	...
	tests := []struct {
		...
		{"log.Infof(a)", 0, true},
		{"log.Errorf(b)", 0, true},
		{"log.Debugf(c)", 0, false},
		{"x = log.Infof(a)", 4, true},
		{"fmt.Printf(a)", 0, false},
	}

	for _, tt := range tests {
		got := rule.Match([]byte(tt.src), tt.pos)
		if got != tt.want {
			t.Errorf("Match(%q, %d) = %v, want %v", tt.src, tt.pos, got, tt.want)
		}
	}
}
...
func TestCallRuleName(t *testing.T) {
	...
	if emptyRule.Name() != "call:unknown" {
		t.Errorf("Name() = %q, want %q", emptyRule.Name(), "call:unknown")
	}
}
...
func TestCallRuleApply(t *testing.T) {
	...
	if string(replacement) != "log.Infof(a, b)" {
		t.Errorf("replacement = %q, want %q", replacement, "log.Infof(a, b)")
	}
}
...
func TestCallRuleApplyNoMatch(t *testing.T) {
	...
	if replacement != nil {
		t.Errorf("replacement = %v, want nil for non-match", replacement)
	}
}
...
func TestMatchPrefixAt(t *testing.T) {
	tests := []struct {
		...
		{"hello world", 0, "hello", true},
		{"hello world", 6, "world", true},
		{"hello", 0, "hello world", false},
		{"hello", 3, "lo", true},
		{"", 0, "x", false},
	}
...
func TestDefaultPatterns(t *testing.T) {
	...
	if len(logPatterns) != 5 {
		t.Errorf("len(DefaultLogPatterns()) = %d, want 5", len(logPatterns))
	}

	fmtPatterns := DefaultFmtPatterns()
	if len(fmtPatterns) != 3 {
		t.Errorf("len(DefaultFmtPatterns()) = %d, want 3", len(fmtPatterns))
	}

	allPatterns := DefaultCallPatterns()
	if len(allPatterns) != 8 {
		t.Errorf("len(DefaultCallPatterns()) = %d, want 8", len(allPatterns))
	}
}
...
func TestRuleMatcherMatchAt(t *testing.T) {
	...
	if matched.Name() != "call:log.Infof(" {
		t.Errorf("Name() = %q, want %q", matched.Name(), "call:log.Infof(")
	}
...
func TestDefaultOperatorRules(t *testing.T) {
	...
	if andPriority <= orPriority {
		t.Errorf("Expected && priority (%d) > || priority (%d)", andPriority, orPriority)
	}
}
